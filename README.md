...
The house box/frame layout, shared by `pi-user-message` (the user's message in
the transcript), `pi-recap` (the recap frame), and `pi-context-footer` (the
prompt frame) — the three boxes that share one layout language, generated from
one place. Two rule dialects plus the rails:

- `frameRuleRow` — the prompt-frame rule: status items as notches flanked by
  fixed rule runs, optional right-anchored trail (`╭── item ── item ────╮`).
- `labelRuleRow` — the transcript-box rule: one label flush against a corner
  (`╭ ● User ────╮`, `╰──── generated by … ╯`).
- `railRow` — content between the rails, exact-width, optional per-row
  background (the `\x1b[49m` trick that survives inner foreground resets).
- `isRuleRow` — recognizes a rule row when hunting for one (the footer's
  autocomplete/scroll handling).

Every row is exactly the requested visible width, or truncated — pi tears the
whole TUI down on an over-wide row, so the invariant is the contract.

```ts
labelRuleRow({ width, paint, cornerL: CORNER_TL, cornerR: CORNER_TR, label, side: "left", padLabel: true });
railRow({ line, paint, padX: 1, padTo: inner, bg: filled });
```
